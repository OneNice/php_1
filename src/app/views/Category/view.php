<div class="content">
    <div class="main">
        <div class="list flex flex-wrap">
            <script>
                $('li[data="<?= $cat1->alias ?>"]').toggleClass('active');
            </script>
            <?php foreach ($events as $event ) {?>
                <article class="<?php if(!$event->price) echo 'free';?>"><?php
...
<script>
    $(window).on('load', function () {
        $('.main').css('min-height', ($(window).outerHeight() - $('header').outerHeight() - $('nav').outerHeight() - $('footer').outerHeight()));
    });
</script>
